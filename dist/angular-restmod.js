...
angular.module('plRestmod').provider('$restmod', function() {
  ...
  return {
    ...
    $get: ['$http', '$q', '$injector', '$parse', '$filter', '$inflector', function($http, $q, $injector, $parse, $filter, $inflector) {
       ...
       * @class CommonApi
       *
       * @description Common behaviour inherited both by {@link Model} and {@link ModelCollection}.
       */
      var CommonApi = {
        ...
        $dispatch: function(_hook, _args, _ctx) {
          ...
          if(this.$$dsp) {
            this.$$dsp(_hook, _args, _ctx);
          }

          // instance callbacks
          if(this.$$cb && (cbs = this.$$cb[_hook])) {
            for(i = 0; !!(cb = cbs[i]); i++) {
              cb.apply(_ctx, _args || emptyArray);
            }
          }
        ...
        $decorate: function(_hooks, _fun) {
          ...
          // set new dispatcher
          this.$$dsp = (typeof _hooks === 'function') ? _hooks : function(_hook, _args, _ctx) {
            if(oldDispatcher) oldDispatcher.apply(null, arguments);
            var extraCb = _hooks[_hook];
            if(extraCb) extraCb.apply(_ctx, _args || emptyArray);
          };
      ...
      var restmod = {
        ...
        model: function(_baseUrl/* , _mix */) {
          ...
          var Collection = {
             ...
             */
            $url: function() {
              return this.$scope ? this.$scope.$url() : Model.$baseUrl();
            },
            ...
            $collection: function(_params, _scope) {
              ...
              col.$scope = _scope || this.$scope || Model;
          ...
          var Builder = {
            ...
            on: function(_hook, _do) {
              CommonApi.$on.call(Model, _hook, _do);
              return this;
            },
          ...
          // CommonApi cant be overriden
          extend(Model, Collection, CommonApi);
          extend(Collection, CommonApi);
