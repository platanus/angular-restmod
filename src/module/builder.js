...
RMModule.factory('RMBuilder', ['$injector', '$parse', '$filter', '$inflector', 'RMUtils', function($injector, $parse, $filter, $inflector, Utils) {
  ...
  function BuilderDSL(_targetModel) {
    ...
    this.$$mappings = {
      ...
      belongsTo: ['attrAsReference', 'key', 'source', 'prefetch']
    };
  }
  ...
  BuilderDSL.prototype = {
     ...
     *
     * Its also posible to override the **foreign key name** or the **inline data property name**.
     *
     ...
     * @param {string} _source inline data porperty name (optional, defaults to _attr), if set to false then inline data is not considered.
     * @param {bool} _prefetch if set to true, $fetch will be automatically called on relation object load.
     * @return {BuilderApi} self
     */
    attrAsReference: function(_attr, _model, _key, _source, _prefetch) {
      ...
      if(_source !== false) {
        if(_source) this.$$s.setMapping(_attr, _source); // set a mapping if explicit source is given.
        this.$$s.setDecoder(_attr, function(_raw) {
      ...
      if(_key !== false) {
        this.$$s.setMapping(_attr + 'Id', _key || '*'); // use dummy  set a mapping if explicit key is given.

        this.$$s.setDecoder(_attr + 'Id', function(_value) {
          if(_value === undefined) return; // if value is undefined, then no id came in request, do nothing.
